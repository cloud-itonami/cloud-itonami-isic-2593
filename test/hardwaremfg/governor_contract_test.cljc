(ns hardwaremfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the forging hammer, grinding
  wheel, heat-treatment furnace or finishing line directly... does NOT
  authorize or execute maintenance/forge-grind-line operations')
  implemented faithfully. The single invariant under test:

    HardwareShopAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Hardware Shop Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :cutlery-item}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :cutlery-item (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-forge-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest forge-grind-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-forge-grind-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:forge-grind-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "forge-001" :maintenance-type :die-inspection
                                          :scheduled-date "2026-08-01" :actuate-forge-grind-line? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-product-category-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-category :unobtainium-gadget}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-category} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-gadget (:product-category (store/batch db "batch-001"))) "fabricated product-category never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "forge-001" :severity :moderate
                                            :description "forging hammer pinch-point guard loose"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "forge-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
