(ns hardwaremfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [hardwaremfg.registry :as r]))
...
(deftest small-shipment-within-weight-does-not-exceed
  (is (false? (r/shipment-weight-exceeded?
               {:weight-kg 1200.0 :shipped-weight-kg 200.0} 500.0))))
...
(deftest known-product-categories-are-valid
  (doseq [c [:cutlery-item :hand-tool-item :general-hardware-item
...
(deftest fabricated-product-category-is-invalid
  (is (not (r/product-category-valid? :unobtainium-gadget)))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "forge-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "forge-001" 1)
