(ns hardwaremfg.phase
  "Phase 0->3 staged rollout for the cutlery/hand-tool/general-hardware
  shop plant-operations coordination actor.
  ...
  consequence (a forging-hammer/grinding-wheel/heat-treatment-furnace/
  finishing-line maintenance window means production downtime, and the
  equipment ends up actually touched); it is always a human shop
  supervisor's call. `hardwaremfg.governor`'s
  `forge-grind-line-actuate-blocked-violations` HARD-blocks actuate
  attempts unconditionally, and the confidence/high-stakes gate
  independently never lets `:flag-safety-concern` auto-commit either
  -- multiple independent layers agree on where this actor's authority
  ends. Like every prior sibling's phase-3 `:auto` set, this domain has
  only ONE member (`:log-production-batch`) -- no separate no-risk
...
(defn verdict->disposition
  "Map a Hardware Shop Plant Operations Governor verdict to a base
