(ns hardwaremfg.advisor
  "HardwareShopAdvisor -- the *contained intelligence node* for the
  cutlery/hand-tool/general-hardware shop plant-operations coordination
  actor (ISIC 2593, manufacture of cutlery, hand tools and general
  hardware).

  It normalizes production-batch patches (product-category/weight/
  defect-rate), drafts a forging-hammer/grinding-wheel/heat-treatment-
  furnace/finishing-line maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound cutlery/hand-tool/general-hardware product shipment
  ...
  a real forging/grinding-line actuation or freight dispatch. Every
  output is censored downstream by `hardwaremfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `hardwaremfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct forging/
                                 ; grinding-line-control effect
  ...
  `hardwaremfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [hardwaremfg.registry :as registry]
            [hardwaremfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch (forged/ground/heat-treated/finished lot) intake
  upsert -- the advisor only normalizes/validates the patch; it does
  not invent the batch's product-category, weight, defect-rate or
...
(defn- schedule-maintenance
  "Draft a forging-hammer/grinding-wheel/heat-treatment-furnace/
  finishing-line maintenance-window scheduling proposal against a
  piece of equipment. The advisor reports what it can see (equipment
  verified?/registered?) in its rationale, but `hardwaremfg.governor`
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-forge-grind-line?=" (boolean (:actuate-forge-grind-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-forge-grind-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a sharp-edge-laceration (freshly forged/ground blade or edge)/
  forging-hammer-pinch-crush/grinding-wheel-abrasive-dust/heat-
  treatment-furnace-burn-radiant-heat/equipment-safety concern. ALWAYS
  ...
  `hardwaremfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `hardwaremfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound cutlery/hand-tool/general-hardware product
  ...
  NOT invent one, and `hardwaremfg.governor` NEVER trusts it: it
...
(def ^:private system-prompt
  (str "あなたは金物(刃物・手工具・一般金物)工場"
       ...
       "鍛造ハンマー・研削盤・熱処理炉等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `hardwaremfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :hardwaremfg-advisor-proposal
   :op         (:op request)
