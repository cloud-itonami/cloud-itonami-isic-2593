(ns hardwaremfg.store
  "SSoT for the cutlery/hand-tool/general-hardware shop plant-
  ...
  (`cloud-itonami-isic-2599`'s own `metalfabmfg.store`), this build
  ...
  self-contained (no external cutlery/hand-tool/hardware capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             forged/ground/heat-treated/finished lot)'s
                             product-category/weight/defect-rate
                             record. `:verified?` marks whether the
                             batch's own claims have actually been
                             QC-inspected (never inferred from a
                             routine intake patch); `:registered?`
                             marks whether it is on file in the shop's
                             production ledger; `:shipped-weight-kg`
                             tracks the batch's own cumulative-shipped
                             ground truth.
    - `equipment`         -- a forging-hammer/grinding-wheel/heat-
                             treatment-furnace/finishing-line unit's
                             own record. `:verified?`/`:registered?`
                             track whether it has actually been
                             inspected/commissioned and is on file --
                             the same ground-truth discipline as
                             `batches`.
    - `maintenance`       -- a scheduled forging/grinding/heat-
                             treatment/finishing-equipment maintenance-
                             window DRAFT against a piece of equipment
                             (`hardwaremfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound cutlery/hand-tool/
                             general-hardware product shipment DRAFT
                             (`hardwaremfg.registry`'s
                             `register-shipment`).
  ...
  audit trail a shop owner or downstream buyer trusting this
  coordinator needs."
  (:require [hardwaremfg.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (hardwaremfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (hardwaremfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :cutlery-item :process-type :forging
                :material "Forged Chef Knife Blade Lot"
                :weight-kg 1200.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-weight-kg 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :hand-tool-item :process-type :grinding
                :material "Ground Claw Hammer Head Lot"
                :weight-kg 6000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-weight-kg 5600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :general-hardware-item :process-type :heat-treatment
                :material "Heat-Treated Door Hinge Lot"
                :weight-kg 3000.0 :defect-rate-percent 1.4
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"forge-001" {:id "forge-001" :kind :forging-hammer
                :verified? true :registered? true
                :last-maintenance-date "2026-05-01"}
   "grind-002" {:id "grind-002" :kind :grinding-wheel
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `hardwaremfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `hardwaremfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered forging-hammer unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  grinding-wheel unit (blocks any maintenance scheduling against it)
