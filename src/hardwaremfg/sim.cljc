(ns hardwaremfg.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean cutlery/hand-tool/
  general-hardware shop through intake -> maintenance scheduling
  ...
  that tries to ACTUATE the forging hammer or grinding wheel directly
  ...
  product category, and a production-batch patch with an implausible
  defect-rate reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on forge-001 (verified, registered, forging hammer -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on forge-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on grind-002 (UNVERIFIED/unregistered grinding wheel -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on forge-001 with :actuate-forge-grind-line? true -> HARD hold, PERMANENT, never reaches a human ==")
