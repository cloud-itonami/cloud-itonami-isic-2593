(ns hardwaremfg.operation
  "HardwareOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (HardwareShopAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Hardware Shop
  Plant Operations Governor (:govern) and the rollout phase gate
  ...
  each operation is auditable and checkpointed. A shop's lifecycle is
  ...
  decision to a human shop supervisor/shipping approver. The approver
  resumes with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hardwaremfg.advisor :as advisor]
...
(defn build
  "Compiles a HardwareOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `hardwaremfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HardwareShopAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Hardware Shop Plant Operations Governor -- independent censor.
      (g/add-node :govern
