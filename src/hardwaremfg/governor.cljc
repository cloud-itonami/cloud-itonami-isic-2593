(ns hardwaremfg.governor
  "Hardware Shop Plant Operations Governor -- the independent
  compliance layer that earns the HardwareShopAdvisor the right to
  ...
  draft-schedule) the forging hammer, grinding wheel, heat-treatment
  furnace or finishing line, whether a shipment proposal's own claimed
  ...
  direct forging/grinding-line-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:hardware-shop-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:forge-hammer/
                                       actuate` or `:grind-wheel/run`)
                                       is the 'direct forging/grinding-
                                       line-equipment control' scope
                                       violation this actor must NEVER
                                       perform -- HARD, PERMANENT,
                                       unconditional.
    4. Forge/grind-line-actuate
       blocked                     -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-forge-grind-
                                       line? true`? Directly actuating
                                       the forging hammer, grinding
                                       wheel, heat-treatment furnace or
                                       finishing line is this actor's
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct forging/
...
(defn- forge-grind-line-control-blocked-violations
  ...
  Anything else (direct forging-hammer/grinding-wheel/heat-treatment-
  furnace/finishing-line control, a fabricated actuation effect) is
  this actor's central scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :forge-grind-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は鍛造ハンマー・研削盤等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- forge-grind-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-forge-grind-line? true` is
  attempting to directly actuate the forging hammer, grinding wheel,
  heat-treatment furnace or finishing line -- this actor may only ever
  propose/schedule a DRAFT maintenance window, never actuate the
  forging/grinding line directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-forge-grind-line? (:value proposal))))
    [{:rule :forge-grind-line-actuate-blocked
      :detail "鍛造ハンマー・研削盤・熱処理炉・仕上げラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn check
  "Censors a HardwareShopAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (forge-grind-line-control-blocked-violations proposal)
                           (forge-grind-line-actuate-blocked-violations request proposal)
